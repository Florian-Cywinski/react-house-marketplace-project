...
function EditListing() {
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      if (docSnap.exists()) {   // if the listing exists
        setListing(docSnap.data())
        setFormData({ ...docSnap.data(), address: docSnap.data().location })  // To put all existing data into the form - to see the state before editing
        setLoading(false)
      } else {
        navigate('/')
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted.current = false // ComponentWillUnmount in Class Component
    }
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(false)
    toast.success('Updated listing saved')
    navigate(`/category/${formDataCopy.type}/${docRef.id}`) // To navigate afterwards to the listing
  }
  ...
  return (
    <div className='profile'>
      ...
      <header>
        <p className='pageHeader centerText'>Edit your Listing</p>
      </header>
